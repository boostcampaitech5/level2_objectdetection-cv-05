...
data_root = 'data/coco/'
img_norm_cfg = dict(
    mean=[110.07, 117.4, 123.65], std=[54.77, 53.36, 54.01], to_rgb=True)
train_pipeline = [
    ...
    dict(
        type='Normalize',
        mean=[110.07, 117.4, 123.65],
        std=[54.77, 53.36, 54.01],
        to_rgb=True),
    dict(
        type='Albu',
        transforms=[dict(type='ChannelShuffle', p=0.5)],
        bbox_params=dict(
            type='BboxParams',
            format='pascal_voc',
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type='CocoDataset',
            ann_file='../../dataset/train_new.json',
            img_prefix='../../dataset/',
            pipeline=[
                dict(type='LoadImageFromFile'),
                dict(type='LoadAnnotations', with_bbox=True)
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(
            type='MMDetWandbHook',
            init_kwargs=dict(
                project='model_test', entity='boostcamp-cv5', name='swin'),
            interval=10,
            log_checkpoint=True,
